...
func (h *Handler) handleIndex() http.HandlerFunc {
	...
	tmpl, err := template.New("base.tmpl").Funcs(funcMap).ParseFS(assets.Files, "templates/base.tmpl", "templates/index.tmpl")
	if err != nil {
		log.Fatal(err)
	}
...
func (h *Handler) handleHello() http.HandlerFunc {
	...
	tmpl, err := template.New("base.tmpl").Funcs(funcMap).ParseFS(assets.Files, "templates/base.tmpl", "templates/hello.tmpl")
	if err != nil {
		log.Fatal(err)
	}
	...
	return func(w http.ResponseWriter, r *http.Request) {
		...
		const name = "license"
		license, err := assets.Files.ReadFile(filepath.Join("templates", name, fmt.Sprintf("%s.%s.html", name, lang)))
		if err != nil {
...
func (h *Handler) handleStaticPage() http.HandlerFunc {
	...
	tmpl, err := template.New("base.tmpl").Funcs(funcMap).ParseFS(assets.Files, "templates/base.tmpl", "templates/page.tmpl")
	if err != nil {
		log.Fatal(err)
	}
	...
	return func(w http.ResponseWriter, r *http.Request) {
		...
		p, err := assets.Files.ReadFile(filepath.Join("templates", name, filename))
		if err != nil {
			Error(w, err, http.StatusNotFound)
			return
		}
...
func translate(key, lang string) template.HTML {
	j, err := assets.Files.ReadFile(filepath.Join("locale", fmt.Sprintf("%s.json", lang)))
	if err != nil {
		log.Println(err)
